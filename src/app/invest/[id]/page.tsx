  ...
  return (
    ...
    >
      ...
      <section className="container mt-24 flex flex-col items-center">
        <h1 className="text-8xl font-bold text-white">{card.title}</h1>
        <h5 className="mt-2.5 text-5xl font-medium text-primary">{card.subtitles}</h5>
      </section>
      <section className="container mt-24">
        <Accordion type="single" collapsible className="w-full">
          {card.Faq?.map((faq, index) => (
            <AccordionItem value={`item-${index}`} className="my-10 w-full" key={index}>
